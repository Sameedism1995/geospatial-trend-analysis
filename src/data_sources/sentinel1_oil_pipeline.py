...
def extract_oil_weekly(
    ...
    *,
    ...
    s1_base_iw = ee.ImageCollection(S1_COLLECTION).filterBounds(bbox).filter(ee.Filter.eq("instrumentMode", "IW"))
    ...
    for wk, start_dt, end_dt in iter_week_utc_bounds(weeks):
        ...
        n_dual = int(ic_dual.size().getInfo())
        n_vh = int(ic_vh.size().getInfo())
...
def main() -> None:
    ...
    run_pipeline(inp, outp, vj, args.buffer_deg, resume=not args.no_resume, debug_log_json=dbg)
